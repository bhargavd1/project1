...
exports.controllerExtensions = {
    manifest: {
        path: () => `sap.ui5/extends/extensions`,
        key: ux_specification_types_1.v4.SAPUI5_CONTROLLER_EXTENSION,
        import: (extensions) => {
            const result = {};
            const controllerExtensions = extensions && typeof extensions === 'object' && ux_specification_types_1.v4.SAPUI5_CONTROLLER_EXTENSION in extensions
                ? extensions[ux_specification_types_1.v4.SAPUI5_CONTROLLER_EXTENSION]
                : {};
            if (controllerExtensions && typeof controllerExtensions === 'object') {
                for (const manifestKey in controllerExtensions) {
                    const configKey = getExtensionKeyForImport(manifestKey);
                    if (configKey) {
                        result[configKey] = {};
                        applyControllerNameProperties(controllerExtensions[manifestKey], result[configKey]);
                    }
                }
            }
        ...
        export: (manifestSection, configObject, key) => {
            ...
            if (!(ux_specification_types_1.v4.SAPUI5_CONTROLLER_EXTENSION in manifestSection) && Object.keys(configExtensions).length) {
                manifestSection[ux_specification_types_1.v4.SAPUI5_CONTROLLER_EXTENSION] = {};
            }
            const controllerExtensions = manifestSection ? manifestSection[ux_specification_types_1.v4.SAPUI5_CONTROLLER_EXTENSION] : {};
            ...
            for (const configKey in configExtensions) {
                ...
                if (!controllerExtensions[manifestKey]) {
                    controllerExtensions[manifestKey] = {};
                }
                applyControllerNameProperties(configExtensions[configKey], controllerExtensions[manifestKey]);
            }
            // Handle removed extensions
            for (const manifestKey in controllerExtensions) {
                const configKey = getExtensionKeyForImport(manifestKey);
                if (configKey && !configExtensions[configKey]) {
                    delete controllerExtensions[manifestKey];
                }
            }
        }
    }
