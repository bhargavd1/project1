...
function fillTableInConfig(importParameters, manifestSettings, configPart, sectionId, factory, tableId) {
    ...
    if (manifestSection) {
        // All table types now use the same ObjectPageTable definition
        configPart[sectionId] = configPart[sectionId] ?? {};
        configPart[sectionId].table = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, ux_specification_types_1.DefinitionName.ObjectPageTable, configPart[sectionId].table);
...
function fillCustomSectionsInConfig(importParameters, manifestSettings, objectPageConfig, factory) {
    ...
    if (customSections) {
        ...
        for (const sectionId in customSections) {
            ...
            if ((0, utils_2.hasEmbeddedComponent)(customSection)) {
                const embeddedSection = customSection;
                const section = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, ux_specification_types_1.DefinitionName.ObjectPageEmbeddedComponentSection);
                (0, utils_1.importSettingsOfObject)(section, importParameters.manifest, importParameters.jsonSchema.definitions?.[ux_specification_types_1.DefinitionName.ObjectPageEmbeddedComponentSection], importParameters.routingId, [], sectionId);
                // Preserve embeddedComponent marker (name only) so App Modeler can identify the section type
                section.embeddedComponent = {
                    name: embeddedSection.embeddedComponent.name
                };
