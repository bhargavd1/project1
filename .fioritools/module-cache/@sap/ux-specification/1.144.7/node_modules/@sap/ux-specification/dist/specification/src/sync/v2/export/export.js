...
function transferCards(params) {
    Object.keys(params.currentObject).forEach((cardKey) => {
        const cardType = params.currentObject[cardKey].template ?? 'CustomCard';
        const cardTypes = [...new Set(Object.keys(ux_specification_types_1.v2.cardTemplateTypeMap))].filter((type) => type !== cardType);
        // Ensure cardTemplate is moved to the end
        cardTypes.push(cardType);
        for (const variantCardType of cardTypes) {
            //Determine target defintion
            let targetDefinition;
            if (!Object.values(ux_specification_types_1.v2.CardTemplateType).includes(variantCardType)) {
                ...
                targetDefinition = Object.keys(params.appSchema.definitions).filter((definition) => {
                    const template = params.appSchema.definitions[definition].properties
                        ?.template;
                    if (template?.enum) {
                        return template.enum.indexOf(variantCardType) > -1;
                    }
                    else if (template?.const) {
                        return template.const === variantCardType;
                    }
                });
            }
            //Get metadata
...
function transferColumnExtensions(transferParameters, customColumnsInConfig, schemaDefinition, objectPageSectionData) {
    ...
    for (const extensionType in ux_specification_types_1.v2.TableColumnExtensionTypeV2) {
        ...
        try {
            ...
            (0, extensionLogger_1.log)(transferParameters.logger, { severity: "error" /* LogSeverity.Error */, message: error.message });
        }
    }
    if (customColumnsInConfig) {
        ...
        for (const path in cluster) {
            (0, controls_1.exportCustomColumnsToFragment)(transferParameters, path, cluster[path].columns, cluster[path].viewTemplateType);
        }
    }
}
/**
