...
function determineTargetAnnotation(ids, transferParameters, configObject) {
    ...
    else if (ids[0] === ux_specification_types_1.PropertyName.chart &&
        [ux_specification_types_1.TemplateType.AnalyticalListPageV4, ux_specification_types_1.TemplateType.ListReportObjectPageV4].includes(transferParameters.templateType)) {
...
function mergeEntriesForInsert(transferParameters, additionalProperties, list, configObject) {
    ...
    // Also add embedded component sections which don't match the custom section fragment
    // (they lack fragmentName but have embeddedComponent). Use the ObjectPageEmbeddedComponentSection
    // definition so the correct @syncRule metadata is applied during export.
    const embeddedDef = transferParameters.appSchema?.definitions?.[ux_specification_types_1.DefinitionName.ObjectPageEmbeddedComponentSection];
    if (embeddedDef) {
        for (const key in configObject) {
            const entry = configObject[key];
            if ((0, utils_3.hasEmbeddedComponent)(entry) && !list.some((item) => item[0] === key)) {
                list.push([
                    key,
                    { $ref: `${common_1.DEFINITION_LINK_PREFIX}${ux_specification_types_1.DefinitionName.ObjectPageEmbeddedComponentSection}` }
                ]);
            }
        }
    }
...
function getClassesForSingleSection(objectPageConfig, sectionId, factory) {
    ...
    if (customSections && customSections[sectionId]) {
        ...
        const isEmbedded = (0, utils_3.hasEmbeddedComponent)(section);
        customSections[sectionId] = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, isEmbedded ? ux_specification_types_1.DefinitionName.ObjectPageEmbeddedComponentSection : 'ObjectPageCustomSectionView', section, isEmbedded ? undefined : section['className']);
    }
}
/**
