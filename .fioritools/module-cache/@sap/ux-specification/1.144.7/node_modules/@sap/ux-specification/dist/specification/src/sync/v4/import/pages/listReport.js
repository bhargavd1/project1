...
/**
 * Import chart toolbar actions of list report.
 *
 * @param importParameters - object comprising all input data
 * @param chartConfig - configuration object for the chart or chart view
 * @param factory - factory, for generating the access to reflect-metadata
 * @param routingId - The unique identifier for the routing configuration in the manifest.
 * @param configId - The unique identifier for the chart configuration in the manifest.
 * @param viewId - The identifier for the specific view in multi-view mode.
 */
function importChartActions(importParameters, chartConfig, factory, routingId, configId, viewId) {
    const schemaDefinitions = importParameters.jsonSchema.definitions;
    const targetAnnotation = configId;
    const actionsDefinition = schemaDefinitions[`${ux_specification_types_1.DefinitionName.Actions}<${viewId}>`];
    if (typeof actionsDefinition === 'object') {
        for (const actionKey in actionsDefinition.properties) {
            const actionDefinition = actionsDefinition.properties[actionKey];
            if (typeof actionDefinition === 'object' && actionDefinition.$ref) {
                ...
                const importActionParams = {
                    ...
                    appSchema: importParameters.jsonSchema,
                    factory,
                    actionsInConfig: chartConfig.toolBar.actions,
                    actionKey,
                    manifest: importParameters.manifest,
                    routingId,
                    targetAnnotation
...
/**
 * Transfer chart toolbar settings of list report.
 *
 * @param importParameters - Object comprising all input data for the import process.
 * @param config - The configuration object to be filled with imported settings.
 * @param factory - Factory instance for generating metadata access objects.
 ...
 * @param routingId - The unique identifier for the routing configuration in the manifest.
 * @param configId - The unique identifier for the chart configuration in the manifest.
 * @param viewId - The identifier for the specific view.
 */
function transferChartToolBar(importParameters, config, factory, pageType, routingId, configId, viewId) {
    const toolbarInstance = factory.createInstance(pageType, ux_specification_types_1.DefinitionName.ViewChartToolBar, { actions: {} });
    let chart;
    if (config && 'chart' in config) {
        chart = config.chart;
    }
    else if (config.table && 'views' in config.table) {
        chart = config.table.views[viewId];
    }
    chart.toolBar = toolbarInstance;
    (0, utils_1.importSettingsOfObject)(chart.toolBar, importParameters.manifest, importParameters.jsonSchema.definitions[ux_specification_types_1.DefinitionName.ViewChartToolBar], routingId, []);
    importChartActions(importParameters, chart, factory, routingId, configId, viewId);
    (0, common_1.removeEmptyStructure)(chart.toolBar, 'actions');
    (0, common_1.removeEmptyStructure)(chart, 'toolBar');
}
/**
...
function importALPViews(importParameters, config, factory, pageType, routingId) {
    ...
    const configId = chartAnnotationPath?.substr(chartAnnotationPath.lastIndexOf('@'));
    ...
    transferChartToolBar(importParameters, config, factory, pageType, routingId, configId, ux_specification_types_1.DefinitionName.ALPChart);
    //table view
...
function importLRViews(importParameters, config, factory, pageType, routingId) {
    ...
    for (const element of sortedList) {
        ...
        if (element[1]['$ref']) {
            ...
            else if (viewType === ux_specification_types_1.DefinitionName.LRChartView) {
                transferChartToolBar(importParameters, config, factory, pageType, routingId, targetAnnotation, element[0]);
            }
